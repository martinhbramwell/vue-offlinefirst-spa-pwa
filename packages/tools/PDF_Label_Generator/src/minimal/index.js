...
const pdfgen = (req, res) => {
  ...
  CLG('+++++++++++++++++++++++++++++++++++++=');
  const theFile = `${testDir}/${testFile}`;
  let doc = new PDF();
  doc.pipe(fs.createWriteStream(`${theFile}`));  //creating a write stream to write the content on the file system
  if (showCornerMarkers) {
      doc.lineJoin('miter')
          .roundedRect(0, 0, 72, 72, 8)
          .stroke();

      doc.lineJoin('miter')
          .roundedRect(537, 720, 72, 72, 8)
          .stroke();
  }
  doc.lineJoin('miter')
      .roundedRect(CNV(5), CNV(5), CNV(15), CNV(10), 8)
      .stroke();
  /* Finalize PDF file */
  doc.end();
  // res.send(`Process complete. Files were written to :: ${tmpDir}/${testFile}`);
  return;
  ...
  codeFiles.forEach((codeFile) => {
    if (creating) {
      ...
      // CLG(`[pdf/index.js] Writing :: ${docName}`);
    ...
    // CLG(` --- wrote :: ${codeFile.code}`);
    // CLG(` --- at (${left_x + positions[pos].x + qrCode_x}, ${top_y + positions[pos].y + qrCode_y})`);
    pos += 1;
  ...
  /* Finalize PDF file */
  doc.end();
  // CLG(`[pdf/index.js] Closed :: ${docName}`);
}
