    ...
    <nav class="panel">
      ...
      <b-tabs v-model="activeSubTab">
        ...
        <b-tab-item :visible="canAssign()">
          ...
          The record for "{{ p(id).nombre }}" will be marked for deletion, will no longer appear in the bottles list, but will be kept for historical purposes.
...
<script>
  ...
  export default {
    ...
    components: {
      'bottle-record': Retrieve, // eslint-disable-line no-undef
      'bottle-update': Update, // eslint-disable-line no-undef
      'bottle-auth': Permissions, // eslint-disable-line no-undef
    },
    methods: {
      canAlter() {
        return this.$can('alter', 'bottle');
      },
      canAssign() {
        return this.$can('assign', 'bottle');
      },
    },
    computed: {
      ...mapGetters('bottle', {
